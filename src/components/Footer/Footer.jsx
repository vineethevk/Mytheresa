import "./Footer.css"


export const Footer = () => {

    return (<>
        <hr />
        <div id="main" >
            <ul></ul>

            <div className="col1" >

                <p> SERVICE & SECURITY </p>

                <h4>Fast Delivery </h4>
                <ul>
                    <li className="li" >Delivery within 2 business days</li>
                </ul>


                <h4>Free returns within 30 days </h4>
                <ul>
                    <li className="li" >
                        Convenient return service</li>
                </ul>

                <h4>Free returns within 30 days </h4>
                <ul>
                    <li className="li" >
                        Convenient return service</li>
                </ul>

                <h4>Secure payment and data protection</h4>
                <ul>
                    <li className="li" >
                        SSL encryption for secure transactions and personal data  </li>
                </ul>

            </div>
            <div className="col2">

                <h4> MYTHERESA </h4>

                <p  >About us </p>
                <p className="li2"> Trust & services </p>
                <p className="li2" > Sustainability </p>
                <p className="li2" > Press </p>
                <p className="li2" > Careers </p>
                <p className="li2" > Investor Relations </p>
                <p className="li2" > Affiliates </p>

            </div>


            <div className="col3" >

                <h4> CUSTOMER CARE </h4>

                <p  >Contact us </p>
                <p className="li2"> Shipping information </p>
                <p className="li2" >Secure payment </p>
                <p className="li2" >Exchanges </p>
                <p className="li2" > Returns </p>
                <p className="li2" >After sale service</p>
                <p className="li2" > Gift card </p>


            </div>

            <div className="col4" >

                <h4> GET TREND UPDATES, STYLE TIPS AND MORE </h4>

                <input type="email" placeholder="your email address here.... " />
                <button className="btn" > SIGN UP </button>

                <p  >Wish List </p>
                <p className="li2" >Receive personal notifications </p>
                <p  >Follow us on </p>

                <div className="media" >
                    <img className="logo" src="data:image/png;base64," alt="" />

                    <img className="logo" src="data:image/png;base64," alt="" />

                </div>

            </div>
        </div>

    </>

    )
}